      ...
      <div className="p-4 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-muted/20">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold tracking-tight">
            Participants <span className="text-muted-foreground ml-1 font-medium">{participants.length}</span>
          </h3>
        </div>
        ...
        <div className="relative group max-w-sm w-full">
          ...
          <input
            ...
            className="w-full h-9 pl-10 pr-4 bg-background border border-border rounded-md text-sm focus:ring-2 focus:ring-primary/10 focus:border-primary outline-none transition-all"
          />
        </div>
      </div>
      ...
      <div className="overflow-x-auto overflow-y-auto max-h-[600px] custom-scrollbar">
        <table className="w-full text-sm text-left sticky-table">
          <thead className="bg-card border-b border-border sticky top-0 z-10">
            <tr>
              <th className="p-4 font-medium text-muted-foreground uppercase tracking-wider text-[10px]">Student Info</th>
              <th className="p-4 font-medium text-muted-foreground uppercase tracking-wider text-[10px]">College ID</th>
              <th className="p-4 font-medium text-muted-foreground uppercase tracking-wider text-[10px]">Status</th>
              <th className="p-4 font-medium text-muted-foreground uppercase tracking-wider text-[10px] text-center">Security</th>
              <th className="p-4 font-medium text-muted-foreground uppercase tracking-wider text-[10px]">Joined At</th>
              <th className="p-4 font-medium text-muted-foreground uppercase tracking-wider text-[10px] text-right">Actions</th>
            </tr>
          </thead>
              ...
              >
                ...
                <td className="p-4 text-center">
                  {p.tab_switch_count && p.tab_switch_count > 0 ? (
                    ...
                    <span className="text-[10px] text-muted-foreground/40 font-mono italic">Secure</span>
                  )}
                </td>
                <td className="p-4">
            ...
            {participants.length === 0 && (
              <tr>
                <td colSpan={6} className="p-20 text-center">
                  ...
                  <p className="text-sm font-bold">Waiting for participants...</p>
                  <p className="text-xs text-muted-foreground mt-1">Share the exam code to let students join the session.</p>
                </td>
              </tr>
            )}
